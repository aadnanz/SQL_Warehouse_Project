...
-- Step 1: Create temp table with source data
IF OBJECT_ID('tempdb..#source_data') IS NOT NULL DROP TABLE #source_data;

SELECT ci.cst_id, ci.cst_key, ci.cst_firstname, ci.cst_lastname, la.cntry, ci.cst_marital_status,
    CASE 
...
FROM #source_data s
INNER JOIN report.dim_customers t
    ON s.cst_id = t.customer_id
    AND t.is_current = 'A'
WHERE 
    ISNULL(t.first_name, '') <> ISNULL(s.cst_firstname, '') OR
    ISNULL(t.last_name, '') <> ISNULL(s.cst_lastname, '') OR
    ISNULL(t.country, '') <> ISNULL(s.cntry, '') OR
    ISNULL(t.marital_status, '') <> ISNULL(s.cst_marital_status, '') OR
    ISNULL(t.gender, '') != ISNULL(s.gender, '') OR
    ISNULL(t.birthdate, '1900-01-01') != ISNULL(s.bdate, '1900-01-01');

-- Step 3: Expire changed records
UPDATE t
SET 
    is_current = 'I',
    modified_dt = CAST(GETDATE() AS DATE)
FROM report.dim_customers t
INNER JOIN #changed_records c
    ON t.customer_id = c.cst_id
WHERE t.is_current = 'A';
...
INSERT INTO report.dim_customers(
    ...
    marital_status, gender, birthdate, create_date, is_current
)
SELECT
    s.cst_id, s.cst_key, s.cst_firstname, s.cst_lastname, s.cntry,
    s.cst_marital_status, s.gender, s.bdate, s.cst_create_date, 'A'
FROM #source_data s
INNER JOIN #changed_records c
...
SELECT
    ...
    s.gender, s.bdate, s.cst_create_date, 'A'
FROM #source_data s
WHERE NOT EXISTS (
    SELECT 1 
    FROM report.dim_customers t 
...
SELECT
    pn.prd_id, pn.prd_key, pn.prd_nm, pn.cat_id, pc.cat, pc.subcat, pc.maintenance, pn.prd_cost,
    pn.prd_line, pn.prd_start_dt
...
-- Step 2: Identify changed records
IF OBJECT_ID('tempdb..#changed_products') IS NOT NULL DROP TABLE #changed_products;

SELECT 
    s.prd_id
INTO #changed_products
FROM #source_products s
INNER JOIN report.dim_products t
    ON s.prd_id = t.product_id
    AND t.is_current = 'A'
WHERE 
    ISNULL(t.product_number, '') <> ISNULL(s.prd_key, '') OR
    ISNULL(t.product_name, '') <> ISNULL(s.prd_nm, '') OR
    ISNULL(t.category_id, '') <> ISNULL(s.cat_id, '') OR
    ISNULL(t.category, '') <> ISNULL(s.cat, '') OR
    ISNULL(t.subcategory, '') <> ISNULL(s.subcat, '') OR
    ISNULL(t.maintenance, '') <> ISNULL(s.maintenance, '') OR
    ISNULL(t.cost, 0) <> ISNULL(s.prd_cost, 0) OR
    ISNULL(t.product_line, '') <> ISNULL(s.prd_line, '') OR
    ISNULL(t.start_date, '1900-01-01') <> ISNULL(s.prd_start_dt, '1900-01-01');
...
INNER JOIN #changed_products c
    ON t.product_id = c.prd_id
WHERE t.is_current = 'A';

-- Step 4: Insert new versions of changed records
INSERT INTO report.dim_products(
    product_id, product_number, product_name, category_id,
    category, subcategory, maintenance, cost, product_line, start_date, is_current
)
SELECT
    s.prd_id, s.prd_key, s.prd_nm, s.cat_id, s.cat, s.subcat, s.maintenance, s.prd_cost,
    s.prd_line, s.prd_start_dt, 'A'
FROM #source_products s
INNER JOIN #changed_products c
    ON s.prd_id = c.prd_id;
...
SELECT
    s.prd_id, s.prd_key, s.prd_nm, s.cat_id, s.cat, s.subcat, s.maintenance,
